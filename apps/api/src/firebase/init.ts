...
if (getApps().length === 0) {
  ...
  if (isDev && !process.env.GOOGLE_APPLICATION_CREDENTIALS && !process.env.FIREBASE_SERVICE_ACCOUNT) {
    // Path to the dummy-credentials.json we created
    process.env.GOOGLE_APPLICATION_CREDENTIALS = path.join(__dirname, '../../dummy-credentials.json')
  }
